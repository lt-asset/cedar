...
def save_seed_for_eagle(seed, workdir, gen_id, save_to_file='eagle_order', file_type='seed', save=False):
    is_dup = False
    ...
    print('============================================')
    print('Saving %s to file %s for EAGLE'  % (file_type, fpath))
    print('============================================')
...
def sync_input_script(fpath):
    ...
    with open(script_record, 'r') as f:
        script_record_lines = sorted(list(set(f.read().splitlines())))
    final_script_record = []
