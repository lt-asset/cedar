...
class Fuzzer:
    ...
    def _check_target_package(self):
        ...
        if mod.__version__ != self.config.version:
            print('WARNING: version specified in config %s does not match with the one in environment %s'
    ...
    def _run_max_iter(self, max_iter, obey, gen_script=False):
        ...
        while True:
            ...
            if count > max_iter and not runforever:
                break
            print('{:#^50}'.format('[%d] %d/%d begin ' % (os.getppid(), count, max_iter)))
            ...
            # new_seed: new input
            # save_flag = self.config.save or (count<=self.config.save_cnt)
            self._test(new_seed, count, obey=obey, gen_script=gen_script, save=self.config.save)
            print('{:#^50}'.format('[%d] %d/%d done ' % (os.getppid(), count, max_iter)))
            print('{:-^50}'.format(''))
    ...
    def _test(self, seed, count=None, obey=True, gen_script=False, save=False):
        ...
        p.start()
        util.DEBUG('Running test in process  '+str(p.pid))
        ...
        if status == Status.PASS and self.config.saved_eagle_cnt < self.config.save_cnt:
            self.config.saved_eagle_cnt += 1
            if not save:
                util.save_seed_for_eagle(seed, self.config.workdir, self.config.saved_eagle_cnt, save=True)
            else:
                util.save_seed_for_eagle(seed, self.config.workdir, self.config.saved_eagle_cnt, save=False)

        if status == Status.PASS or status == Status.FAIL:
            return status
        
        # if not save no every iteration, then save the crash inputs
        if not save:
