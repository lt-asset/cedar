...
class Param:
    ...
    def __init__(self, name, spec, dtype_map, required=True, guided_mutate=False, special_value=False):
        ...
        # remove variance
        # self.var_dep = sorted(list(self.var_dep))
        # self.const_dep = sorted(list(self.const_dep))
    ...
    def _verify_dtypes(self, dtype_map):
        default_dtypes = [k for k in dtype_map.keys() if dtype_map[k] is not None]
        ...
        rest_dtypes = sorted(list(rest_dtypes))
        ...
        rest_dtypes = set(rest_dtypes) - set(special_dtypes) - set(known_dtypes)
        rest_dtypes = list(rest_dtypes)
        if rest_dtypes:
            ...
            if self.can_disobey_dtype:
                if self.valid_dtypes == default_dtypes:
                    ...
                    if dtype_dep is None:  # no dtype dependency
                        pick_dtype = np.random.choice(sorted(list(set(default_dtypes) - set(self.valid_dtypes))))
    ...
    def _pick_disobey_ndim(self, default_ndims):
        ...
        if self.can_disobey_ndim:
            if self.valid_ndims == default_ndims:
                ...
                candidate_ndims = list(set(default_ndims) - set(self.valid_ndims))
                if len(candidate_ndims)==0:
                    candidate_ndims=default_ndims
                pick_ndim = np.random.choice(candidate_ndims)
    ...
    def _gen_value_from_var_dep(self, ref, relation, gen_shape=True):
        ...
        if relation == 'dtype':
            try:
                ...
                if isinstance(dep_param_obj.data, str):
                    return 'str'
