...
class Range:
    ...
    def set_boundary(self, dtype):
        ...
        # self.boundary =  sorted(list(set(self.boundary)))
        # self.boundary = list(dict.fromkeys(self.boundary))
        self.boundary = set(self.boundary)
...
class Mutator:
    ...
    def init_coverage_record(self):
        coverage_record = {}
        for op in sorted(list(self.funcdict.keys())):
    ...
    def update_coverage(self, op, target_dim=None):
        if not self.zero_ndim_covered and op == 'zero_ndim':
            ...
            if not [x for x in self.zero_ndim_record.keys() if self.zero_ndim_record[x]==False]:
        ...
        if not self.mutation_op_covered:
            ...
            if not [x for x in self.coverage_record.keys() if self.coverage_record[x]==False]:
    ...
    def pick_mutation_op(self, max_coverage=True):
        ...
        def _aval_mutator():
            ...
            ret = []
            for op in self.mutator_choices.keys():
    ...
    def pick_target_dim(self):
        ...
        for dim in [x for x in self.zero_ndim_record.keys() if self.zero_ndim_record[x]==False]:
    ...
    def gen_boundary_shape(self, shape, dtype):
        ...
        # len(self.shape) >=2
        while True:
            candidate_dim = [d for d in self.zero_ndim_record.keys() if self.zero_ndim_record[d]==False]
