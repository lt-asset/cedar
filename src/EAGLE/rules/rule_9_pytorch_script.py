    ...
    try:
        if functional:
            ...
            if torch.max(image_input) > 1.001:
                ...
                image_input /= 255.0
                argument_2["mean"] = torch.tensor(argument["mean"])/255.0
                argument_2["std"] = torch.tensor(argument["std"])/255.0
            else:
            ...
            if torch.max(input) > 1.001:
                # if the image is between 0 and 255, then normalize it to [0.0, 1.0]
                input /= 255.0
                argument["mean"] = torch.tensor(argument["mean"])/255.0
                argument["std"] = torch.tensor(argument["std"])/255.0
            else:
                # if the image is between 0.0 and 1.0, then normalize it to [0, 255]
                input *= 255.0
                argument["mean"] = torch.tensor(argument["mean"])*255.0
...
def run(in_dir, out_dir, lib, version, api_config, input_index):
    ...
    if functional:
        argument["img"] = torch.clamp(argument["img"], 0.0, 255.0)
    else:
        input_data = torch.clamp(input_data, 0.0, 255.0)
